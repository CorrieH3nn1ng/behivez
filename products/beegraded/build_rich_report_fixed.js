...
function gauge(label, score, size) {
  ...
  return '<div class="gauge-item" style="text-align:center;flex:1;min-width:120px;margin:8px;cursor:pointer;transition:transform 0.15s" onclick="toggleScoreDetail(\x27' + key + '\x27)" title="Click for details">' +
...
var cssBlock = '<style>' +
  ...
  '.cover{background:linear-gradient(135deg,#0f172a 0%,#1e1b4b 40%,#312e81 70%,#4f46e5 100%);padding:24px 20px;text-align:center;position:relative;overflow:hidden}' +
  ...
  '.cover h1{font-size:28px;font-weight:800;background:linear-gradient(135deg,#fbbf24,#f59e0b);-webkit-background-clip:text;-webkit-text-fill-color:transparent;margin-bottom:4px}' +
  '.cover .sub{font-size:14px;color:#94a3b8;margin-bottom:12px}.cover .sbadge{display:inline-block;padding:4px 14px;background:rgba(251,191,36,0.15);border:1px solid rgba(251,191,36,0.3);border-radius:20px;color:#fbbf24;font-weight:600;font-size:13px;margin-bottom:10px}' +
  ...
  '.tab{padding:10px 16px;cursor:pointer;color:#64748b;font-size:12px;font-weight:600;text-transform:uppercase;letter-spacing:0.5px;border-bottom:2px solid transparent;margin-bottom:-2px;transition:all 0.2s}' +
  '.tab:hover{color:#94a3b8}.tab.active{color:#fbbf24;border-bottom-color:#fbbf24}.panel{display:none;max-width:100%;margin:0 auto;padding:16px 12px}' +
  '.panel.active{display:block}.card{background:#0f172a;border:1px solid #1e293b;border-radius:12px;padding:16px;margin-bottom:12px}' +
  '.gauges{display:flex;flex-wrap:wrap;justify-content:center;gap:4px}table{width:100%;border-collapse:collapse;font-size:13px;table-layout:fixed}' +
  'th{padding:8px 6px;text-align:left;background:#1e293b;color:#fbbf24;font-size:11px;text-transform:uppercase;letter-spacing:0.5px}' +
  'td{color:#cbd5e1;word-wrap:break-word;overflow-wrap:break-word}' +
  '.issue-table th:nth-child(1),.issue-table td:nth-child(1){width:15%}' +
  '.issue-table th:nth-child(2),.issue-table td:nth-child(2){width:10%}' +
  '.issue-table th:nth-child(3),.issue-table td:nth-child(3){width:45%}' +
  '.issue-table th:nth-child(4),.issue-table td:nth-child(4){width:30%}' +
  '.ref-table th:nth-child(1),.ref-table td:nth-child(1){width:40%}' +
  '.ref-table th:nth-child(2),.ref-table td:nth-child(2){width:15%}' +
  '.ref-table th:nth-child(3),.ref-table td:nth-child(3){width:15%}' +
  '.ref-table th:nth-child(4),.ref-table td:nth-child(4){width:30%}' +
  '.summary{white-space:pre-wrap;color:#cbd5e1;font-size:14px;line-height:1.8}.stat-row{display:flex;flex-wrap:wrap;gap:10px;margin-bottom:12px}' +
  '.stat{flex:1;min-width:120px;background:#0f172a;border:1px solid #1e293b;border-radius:10px;padding:12px;text-align:center}' +
  '.stat .val{font-size:24px;font-weight:800}.stat .lbl{font-size:10px;color:#64748b;text-transform:uppercase;letter-spacing:0.5px;margin-top:2px}' +
  '.ai-badge{display:inline-block;padding:3px 10px;background:rgba(139,92,246,0.2);border:1px solid rgba(139,92,246,0.3);border-radius:6px;color:#a78bfa;font-size:11px;font-weight:600;margin-top:8px}' +
  '.footer{text-align:center;padding:16px;color:#475569;font-size:11px;border-top:1px solid #1e293b;margin-top:20px}' +
  '@media print{.tabs{display:none}.panel{display:block !important}body{background:white;color:#1e293b}' +
  '.card{border-color:#e2e8f0}.cover{background:#1e293b !important}}@media(max-width:640px){.tab{padding:8px 10px;font-size:11px}' +
  '.gauges>div{min-width:80px}}.gauge-item:hover{transform:scale(1.05)}.gauge-item:active{transform:scale(0.95)}' +
...
var scriptBlock = '<script>' +
  ...
  'var info=d[key]||["",""];' +
  'var score=0;' +
  'document.querySelectorAll(".gauge-item").forEach(function(g){' +
  'var oc=g.getAttribute("onclick")||"";' +
  'if(oc.indexOf("\\x27"+key+"\\x27")>-1){' +
  'var t=g.querySelector("div[style*=\\"font-weight:700\\"]");' +
  'if(t)score=parseInt(t.textContent);}});' +
  'var c=score>=75?"#22c55e":score>=60?"#f59e0b":score>=50?"#fb923c":"#ef4444";' +
...
var html = '<!DOCTYPE html><html lang="en"><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width,initial-scale=1.0"><title>BeeGraded Report</title>' +
cssBlock + '</head><body>' +

'<div class="cover">' +
'<div style="display:flex;align-items:center;justify-content:center;gap:20px;flex-wrap:wrap">' +
'<div style="display:inline-block;width:80px;height:80px;position:relative">' +
...
'<div style="font-size:22px;font-weight:800;color:' + scoreColor + '">' + overall + '</div>' +
'<div style="font-size:8px;color:#94a3b8;letter-spacing:1px">' + gradeLabel + '</div></div></div>' +
'<div style="text-align:left">' +
'<h1 style="font-size:24px;font-weight:800;background:linear-gradient(135deg,#fbbf24,#f59e0b);-webkit-background-clip:text;-webkit-text-fill-color:transparent;margin-bottom:2px">BeeGraded</h1>' +
...
'<div class="card" style="overflow-x:auto"><table class="issue-table">' +
...
'<div class="card" style="overflow-x:auto"><table class="ref-table">' +
